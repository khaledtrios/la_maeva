...
export default function ReturnEdit() {
    ...
    const [returnLines, setReturnLines] = useState<Line[]>(() => {
        if (!productReturn?.lines) return [];
        return productReturn.lines.map((line: any) => ({
            id: line.id,
            reception_line_id: line.reception_line_id,
            product_id: line.product_id,
            product: line.product,
            quantite_attendue: line.quantite_attendue,
            quantite_retournee: line.quantite_retournee,
            dlc: line.dlc || null,
            lot_reference: line.lot_reference || '',
            cause: line.cause,
            notes: line.notes || '',
            origine: line.reception_line_id ? 'auto' : 'manuel',
        }));
    });
    ...
    const [globalNotes, setGlobalNotes] = useState(productReturn?.notes || '');
    ...
    const addManualLine = () => {
        ...
        setReturnLines((prev) => [
            ...prev,
            {
                ...
                product,
                quantite_attendue: newQteRetour,
    ...
    const submit = (e: React.FormEvent) => {
        ...
        router.put(returnsRoutes.update.url(productReturn.id), formData, {
    ...
    return (
        <>
            <Head title={`Modifier le retour ${productReturn.reference}`} />
            <div className="rc-page">
                {/* ── HEADER ── */}
                <div className="rc-header">
                    <Link
                        href={returnsRoutes.show(productReturn.id)}
                        className="rc-back"
                    >
                        <ArrowLeft size={16} strokeWidth={1.5} />
                        <span>Retour {productReturn.reference}</span>
                    </Link>
                    <div className="rc-header-main">
                        <h1 className="rc-title">Modifier le retour</h1>
                        <p className="rc-subtitle">
                            Modifiez les produits et quantités avant envoi
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={submit} className="rc-form">
                    ...
                    <div className="rc-card">
                        ...
                        {expiredStock && expiredStock.length > 0 && (
                            <div className="rc-card-footer rc-expired-section">
